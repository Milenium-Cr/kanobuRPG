...
class Game:
    ...
    def battle(self):
        ...
        print(locale["battle"][0].format(teamlevel=teamlevel, botlevel=botlevel))
        while True:
    ...
    def step(self, action):
        if action == "attack":
            damage = (randint(self.playerchoice.minattack, self.playerchoice.maxattack)) * self.checkweakness("player") - self.botchoice.defence
            if damage <= 0:
                damage = 1
            ...
            damage = (randint(self.botchoice.minattack, self.botchoice.maxattack)) * self.checkweakness("bot") - self.playerchoice.defence
            if damage <= 0:
                damage = 1
    ...
    def level_up(self):
        while True:
            if self.exp >= self.levelup_exp:
                ...
                self.level += 1
                health_up = randint(data["modifications"]["statsup"]["health"]["min"], data["modifications"]["statsup"]["health"]["max"])
                minattack_up = randint(data["modifications"]["statsup"]["attack"]["min"], data["modifications"]["statsup"]["attack"]["max"])
                maxattack_up = randint(data["modifications"]["statsup"]["attack"]["min"], data["modifications"]["statsup"]["attack"]["max"])
                defence_up = randint(data["modifications"]["statsup"]["defence"]["min"], data["modifications"]["statsup"]["defence"]["max"])
                levelexp_up = randint(data["modifications"]["statsup"]["exp"]["min"], data["modifications"]["statsup"]["exp"]["max"])
